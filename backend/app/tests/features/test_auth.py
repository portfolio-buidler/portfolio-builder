...
async def test_refresh_token_flow(client: AsyncClient):
    ...
    # Try to refresh (endpoint may be /auth/refresh)
    refresh_response = await client.post("/auth/refresh")
    
    # If endpoint exists, should return new access token
...
async def test_concurrent_registrations_same_email(client: AsyncClient):
    ...
    # Exactly one should succeed (201), others should fail (401)
    success_count = sum(1 for r in responses if hasattr(r, 'status_code') and r.status_code == 201)
    fail_count = sum(1 for r in responses if hasattr(r, 'status_code') and r.status_code == 401)
    
    assert success_count == 1, "Exactly one registration should succeed"
    assert fail_count == 2, "Two registrations should fail with 401"
...
async def test_user_email_case_insensitive(client: AsyncClient):
    ...
    dup_response = await client.post("/auth/register", json=registration_data2)
