...
def read_docx_text(path: Path | str) -> str:
    ...
    Many résumé templates embed the candidate’s name and contact details in a header.  When the
    header is processed after the main document the extracted text will place the name at the end
    of the document, confusing downstream heuristics.  To avoid this, headers are extracted
    **before** the main document, followed by any footers.
    """
    p = Path(path)
    with ZipFile(p) as zf:
        # Build a list of parts to extract.  Process headers first, then the main document, then footers.
        names: list[str] = []
